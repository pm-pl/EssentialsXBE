...
class ExpCommand extends Command implements PluginOwned {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        if (!$sender instanceof Player) {
            $sender->sendMessage(Utils::getConfig()->getNested("xp.in_game_only", "&cThis command must be used in-game."));
            return;
        }
        if (empty($args)) {
            ...
            $message = Utils::getConfig()->getNested("xp.self_info", "{player} §r§6has §r§c{exp} EXP §r§6(level §r§c{level}§r§6) §r§6and needs {levelup} more exp to level up.");
        ...
        switch ($args[0]) {
            case 'add':
                if (!$sender->hasPermission('essentialsx.xp.add')) {
                    $message = Utils::getConfig()->getNested("xp.no_permission", "&cYou do not have permission to use this command.");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                if (count($args) !== 3) {
                    $message = Utils::getConfig()->getNested("xp.add_usage", "&cUsage: /xp add <player> <amount>");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                if (!$player) {
                    $message = Utils::getConfig()->getNested("xp.player_not_found", "&cPlayer not found.");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                $amount = (int) $args[2];
                if ($amount <= 0) {
                    $message = Utils::getConfig()->getNested("xp.invalid_amount", "&cAmount must be a positive integer.");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                $message = Utils::getConfig()->getNested("xp.add_success", "&aAdded {amount} XP to {player}. Their new XP is {new_xp}.");
            ...
            case 'remove':
                ...
                if (count($args) !== 3) {
                    $message = Utils::getConfig()->getNested("xp.remove_usage", "&cUsage: /xp remove <player> <amount>");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                if ($amount > $currentXp) {
                    $message = Utils::getConfig()->getNested("xp.insufficient_xp", "&c{player} does not have that much XP.");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                $message = Utils::getConfig()->getNested("xp.remove_success", "&aRemoved {amount} XP from {player}. Their new XP is {new_xp}.");
                $message = str_replace("{amount}", number_format($amount), $message);
                $message = str_replace("{player}", $player->getName(), $message);
                $message = str_replace("{new_xp}", number_format($newXp), $message);
                $message = str_replace("&", "§", $message); // Allowing color codes with '&'
            ...
            case 'set':
                ...
                if (count($args) !== 3) {
                    $message = Utils::getConfig()->getNested("xp.set_usage", "&cUsage: /xp set <player> <amount>");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                $amount = (int) $args[2];
                if ($amount < 0) {
                    $message = Utils::getConfig()->getNested("xp.invalid_amount", "&cAmount must be a non-negative integer.");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                $player->getXpManager()->setCurrentTotalXp($amount);
                $message = Utils::getConfig()->getNested("xp.set_success", "&aSet {player}'s XP to {amount}.");
                $message = str_replace("{player}", $player->getName(), $message);
                $message = str_replace("{amount}", number_format($amount), $message);
                $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                $sender->sendMessage($message);
            ...
            case 'show':
                ...
                if (count($args) !== 2) {
                    $message = Utils::getConfig()->getNested("xp.show_usage", "&cUsage: /xp show <player>");
                    $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                    $sender->sendMessage($message);
                    return;
                }
                ...
                $message = Utils::getConfig()->getNested("xp.show_info", "{player} §r§6has §r§c{xp} EXP §r§6(level §r§c{level}§r§6) §r§6and needs {levelup} more exp to level up.");
                ...
                break;
            default:
                $message = Utils::getConfig()->getNested("xp.invalid_command", "&cUsage: /xp [add|remove|set|show] <player> <amount>");
                $message = str_replace("&", "§", $message); // Allowing color codes with '&'
                $sender->sendMessage($message);
                break;
        }
